#!/usr/bin/env bash
# Helper script for managing auto-penguin-setup development containers
...
print_usage() {
  cat <<EOF
Auto-Penguin-Setup Container Manager
...
cmd_shell() {
  ...
  local container_name="auto-penguin-${distro}"

  echo -e "${BLUE}Opening shell in ${distro} container...${NC}"
  echo -e "${YELLOW}Tip: Run 'cp -r /workspace ~/auto-penguin-setup && cd ~/auto-penguin-setup' to get started${NC}"
  echo ""
...
cmd_clean() {
  ...
  if [[ "$distro" == "all" ]]; then
    ...
    podman volume rm "container_${distro}-config" 2>/dev/null || true
...
cmd_status() {
  ...
  podman ps -a --filter "name=auto-penguin" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"

  echo ""
  echo -e "${BLUE}Volumes:${NC}"
  podman volume ls --filter "name=container_" --format "table {{.Name}}\t{{.Driver}}\t{{.Mountpoint}}"
}
